...
## Model ##
from lib.IncrementalEnergyCAModel import IncrementalEnergyCAModel, IncrementalEnergyCAModelTrainer, IncrementalEnergyCAModelVisualizer
    
# grid search on beta_energy
beta_energies = [1e-9]#[1e-9, 2e-9, 5e-9, 2e-10, 5e-10,0]
for beta_energy in beta_energies:
    energyca_params['BETA_ENERGY'] = beta_energy
    ca = IncrementalEnergyCAModel(global_params['CHANNEL_N'], training_params['device'])
    ...
    model_name = f"IncrementalEnergyCA_gumbel_EnergyLoss_variablebetaEnergy{energyca_params['BETA_ENERGY']:.0e}_maxSteps{global_params['MAX_STEPS']}"

    ## Training ##
    print("Currently training:", model_name)
    train_ca(ca, IncrementalEnergyCAModelTrainer, 
            ...
            Visualizer = IncrementalEnergyCAModelVisualizer, visualize_step=100, figs_dir='figs/')

# run this script with output to a file, and delete the previous output file
# nohup python trainIncrementalEnergyCA.py > trainIncrementalEnergyCA.log 2>&1 &

# delete every file in models/, histories/, and figs/
# rm -rf models/* histories/* figs/*

# print last 10 lines of a file
# tail -n 10 trainIncrementalEnergyCA.log
